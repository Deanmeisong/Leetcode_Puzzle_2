...
public:
    int earliestSecondToMarkIndices(vector<int>& nums, vector<int>& changeIndices) {
        n = nums.size(); m = changeIndices.size();
        nums.insert(nums.begin(), 0); changeIndices.insert(changeIndices.begin(), 0);
        int left = 1, right = m;
        while(left < right) {
            int mid = (right - left)/2 + left;
            if(isOK(mid, nums, changeIndices)) right = mid;
            else left = mid + 1;
        }
        if(isOK(left, nums, changeIndices)) return left;
        else return -1;
    }
    ...
    bool isOK(int t, vector<int>& nums, vector<int>& changeIndices) {
        ...
        for(int i = 1; i <= t; ++i) {
            if(first[changeIndices[i]] == 0 && nums[changeIndices[i]] != 0) first[changeIndices[i]] = i;
            else changeIndices[i] = 0;     
        }
