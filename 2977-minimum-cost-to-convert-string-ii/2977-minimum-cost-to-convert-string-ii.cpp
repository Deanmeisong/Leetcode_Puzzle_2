...
public:
    ...
    long long minimumCost(string source, string target, vector<string>& original, vector<string>& changed, vector<int>& cost) {
        ...
        for(int i = 1; i <= m; ++i) {
            ...
            TrieNode* node1 = root; TrieNode* node2 = root;
            for(int j = i; j >= 1; --j) {
                if(node1->next[source[j]-'a'] == NULL) break;
                if(node2->next[target[j]-'a'] == NULL) break;
                node1 = node1->next[source[j]-'a']; node2 = node2->next[target[j]-'a'];
                int idx1 = node1->idx; int idx2 = node2->idx;
